// @ts-nocheck
import { Color, DoubleSide, LinearSRGBColorSpace, Matrix4, MeshPhysicalMaterial, Quaternion, SRGBColorSpace, Vector2 } from 'three';
...
export class ThreeRenderDelegateMaterialOps extends ThreeRenderDelegateCore {
  getActiveStageRootPrimPath() {
    const snapshotDefaultPrimPath = normalizeHydraPath(
      this.getCachedRobotSceneSnapshot?.()?.stage?.defaultPrimPath || '',
    );
    if (snapshotDefaultPrimPath) return snapshotDefaultPrimPath;

    const stageDefaultPrimPath = normalizeHydraPath(
      this.getStage?.()?.GetDefaultPrim?.()?.GetPath?.()?.pathString || '',
    );
    if (stageDefaultPrimPath) return stageDefaultPrimPath;
  ...
  getPrimPathIfWithinActiveStageRoot(primPath) {
    ...
    if (!activeStageRootPrimPath) return normalizedPrimPath;
    if (normalizedPrimPath === activeStageRootPrimPath) return normalizedPrimPath;
    if (normalizedPrimPath.startsWith(`${activeStageRootPrimPath}/`)) return normalizedPrimPath;
    return null;
  }

  shouldSuppressSyntheticTopLevelMesh(meshId) {
    const normalizedMeshId = normalizeHydraPath(meshId || '');
    if (!normalizedMeshId || normalizedMeshId.includes('.proto_')) return false;
    if (!/^\/(?:meshes|visuals|colliders?|collision)(?:$|[/.])/i.test(normalizedMeshId)) return false;

    const activeStageRootPrimPath = this.getActiveStageRootPrimPath?.();
    if (!activeStageRootPrimPath) return false;
    if (this.getPrimPathIfWithinActiveStageRoot(normalizedMeshId)) return false;
  ...
  pruneSyntheticTopLevelMeshes() {
    ...
    if (meshEntries.length === 0) return 0;

    let removedCount = 0;
    for (const [meshId, hydraMesh] of meshEntries) {
      if (!this.shouldSuppressSyntheticTopLevelMesh?.(meshId)) continue;
  ...
  getPrimAtPathAllowUnknown(stage, primPath) {
    ...
    if (!stage || !normalizedPath) return null;

    const safePrim = this.safeGetPrimAtPath(stage, normalizedPath);
    if (safePrim) return safePrim;
    ...
    try {
      if (typeof stage.GetPrimAtPath !== 'function') return null;
      const directPrim = stage.GetPrimAtPath(normalizedPath);
      if (!directPrim) return null;
  ...
  resolveLegacyCollisionPrimPathFromStage(meshPath) {
    ...
    const addCandidatePath = (candidatePath) => {
      ...
      if (!normalizedCandidatePath || seenCandidatePaths.has(normalizedCandidatePath)) return;
    ...
    if (this._knownPrimPathSet && typeof this._knownPrimPathSet[Symbol.iterator] === 'function') {
      for (const knownPath of this._knownPrimPathSet) {
        ...
        if (!normalizedKnownPath || !normalizedKnownPath.startsWith(`${normalizedMeshPath}/`)) continue;
  ...
  getCollisionSemanticMeshCandidatesFromKnownSet(containerPath) {
    ...
    if (!normalizedContainerPath || !normalizedContainerPath.startsWith('/')) return [];
    ...
    if (!knownPathSet || knownPathSet.size === 0) return [];
    ...
    for (const rawPath of knownPathSet) {
      ...
      if (!normalizedPath || !normalizedPath.startsWith(prefix) || !normalizedPath.endsWith('/mesh')) continue;
      const relativePath = normalizedPath.slice(prefix.length);
      const parts = relativePath.split('/');
      if (parts.length !== 2) continue;
      const childName = String(parts[0] || '');
      if (!childName || /^mesh_\d+$/i.test(childName)) continue;
  ...
  resolveCollisionMeshPrimPathFromKnownSet(meshId, overridePrimPath = null) {
    ...
    const proto = parseProtoMeshIdentifier(meshId);
    if (!proto || proto.sectionName !== 'collisions' || proto.protoType !== 'mesh') return null;
    ...
    if (peerMeshIds.length === 0) return null;
    ...
    for (const peerMeshId of peerMeshIds) {
      ...
      if (!peerPath || !semanticCandidateSet.has(peerPath)) continue;
  ...
  getResolvedPrimPathForMeshId(meshId) {
    ...
    if (this._resolvedProtoPrimPathCache.has(meshId)) {
      const cachedPath = this.getPrimPathIfWithinActiveStageRoot(
        this._resolvedProtoPrimPathCache.get(meshId) || null,
      );
  ...
  refreshMeshStageOverrides(options = {}) {
    ...
    const finalStageBatchCoverageLikelyFull = (
      finalStageBatchEnabled
    ...
    for (let meshIndex = startIndex; meshIndex < protoMeshes.length; meshIndex++) {
      ...
      try {
        ...
        if (!appliedByFinalStageBatch) {
          ...
          const needsResolvedPrimFallback = (
            isCollisionProto
            && primPath
            && !shouldSkipCollisionReapply
  ...
  normalizeSnapshotMaterialRecords(rawRecords, options = {}) {
    ...
    const normalizeScalar = (value, config = {}) => {
      ...
      if (config.clamp01) normalized = clamp01(normalized);
      if (Number.isFinite(config.min)) normalized = Math.max(Number(config.min), normalized);
      if (Number.isFinite(config.max)) normalized = Math.min(Number(config.max), normalized);
      return normalized;
    };
  ...
  ingestSnapshotMaterialRecords(rawRecords, options = {}) {
    ...
    for (const materialId of stageMaterialIds) {
      if (this.materials[materialId]) continue;
  ...
  applySnapshotTextureInput(material, texturePath, materialProperty, options = {}) {
    ...
    if (!material || !normalizedTexturePath) return false;
  ...
  applySnapshotMaterialRecord(material, record) {
    if (!material || !record || typeof record !== 'object') return;
  ...
  createFallbackMaterialFromSnapshot(materialPath) {
    ...
    if (!normalizedMaterialPath) return null;
    if (!(this._snapshotMaterialRecordById instanceof Map)) return null;
    if (!(this._snapshotFallbackMaterialCache instanceof Map)) {
      this._snapshotFallbackMaterialCache = new Map();
    }
  ...
  applySnapshotMaterialsToMeshes() {
    ...
    for (const hydraMesh of Object.values(this.meshes || {})) {
      ...
      const pendingMaterialId = normalizeHydraPath((hydraMesh as any)._pendingMaterialId || '');
      if (!pendingMaterialId) continue;
      const resolvedMaterial = this.materials?.[pendingMaterialId]?._material || null;
      if (!resolvedMaterial || !(hydraMesh as any)._mesh) continue;
      (hydraMesh as any)._mesh.material = resolvedMaterial;
      (hydraMesh as any)._pendingMaterialId = undefined;
      summary.boundCount += 1;
    }
    for (const hydraMesh of Object.values(this.meshes || {})) {
      if (!hydraMesh || typeof hydraMesh !== 'object') continue;
      try {
        if ((hydraMesh as any).tryApplyPendingGeomSubsetMaterials?.() === true) {
    ...
    for (const hydraMesh of Object.values(this.meshes || {})) {
      if (!hydraMesh || typeof hydraMesh !== 'object') continue;
      try {
        if ((hydraMesh as any).tryInheritVisualMaterialFromLink?.() === true) {
