...
type QuaternionWxyzTuple = [number, number, number, number];
...
export function getAngleInRadians(value: number): number {
  return value * Math.PI / 180;
}

export function createMatrixFromXformOp(opName: string, value: unknown): Matrix4 | null {
  const opMatrix = new Matrix4();
  const arrayValue = toArrayLike(value) as ArrayLike<unknown> | null;

  if (opName.startsWith('xformOp:transform')) {
    if (!arrayValue || arrayValue.length < 16) return null;
  ...
  if (opName.startsWith('xformOp:translate')) {
    const translate = toFiniteVector3Tuple(value) as Vector3Tuple | null;
  ...
  if (opName.startsWith('xformOp:scale')) {
    const scale = toFiniteVector3Tuple(value) as Vector3Tuple | null;
  ...
  if (opName.startsWith('xformOp:rotateXYZ')) {
    const rotate = toFiniteVector3Tuple(value) as Vector3Tuple | null;
  ...
  if (opName.startsWith('xformOp:orient')) {
    const orientation = toFiniteQuaternionWxyzTuple(value) as QuaternionWxyzTuple | null;
...
export function normalizeHydraPath(value: unknown): string {
  if (value === null || value === undefined) return '';
  if (typeof value === 'string') return value;
  const pathLike = value as HydraPathLike;
...
export function nearlyEqual(a: number, b: number, epsilon = 1e-4): boolean {
  return Math.abs(Number(a) - Number(b)) <= epsilon;
}

export function toColorArray(value: unknown): [number, number, number] | null {
  const arrayValue = toArrayLike(value) as ArrayLike<unknown> | null;
  if (!arrayValue || arrayValue.length < 3) return null;
