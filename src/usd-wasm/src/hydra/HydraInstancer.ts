...
type NumericArrayLike = ArrayLike<number> | null;

type HydraMeshLike = {
  _instancerId?: string | null;
...
class HydraInstancer {
  private _id: string;
  private _interface: HydraInterfaceLike;
  private _prototypes: unknown[] = [];
  private _transforms: Matrix4[] = [];
  private _indices: number[] = [];
  private _instancedMeshes: InstancedMesh[] = [];
  private _translate: NumericArrayLike = null;
  private _rotate: NumericArrayLike = null;
  private _scale: NumericArrayLike = null;
  private _instanceTransforms: NumericArrayLike = null;
  private _instanceIndices: NumericArrayLike = null;

  constructor(id: unknown, hydraInterface: HydraInterfaceLike) {
    this._id = normalizeHydraPath(id);
    this._interface = hydraInterface;

    if (debugInstancer) console.log("Created HydraInstancer", id);
  }

  updatePrimvar(name: string, data: NumericArrayLike, dimension: number, interpolation: string): void {
    if (debugInstancer) console.log(`Instancer ${this._id} updatePrimvar: ${name}`, dimension, interpolation);
  ...
  commit(): void {
    if (debugInstancer) console.log(`Committing Instancer ${this._id}`);
    ...
    if (prototypeIds.length === 0) {
        if (debugInstancer) console.warn(`Instancer ${this._id} has no prototypes.`);
        return;
    }

    let instanceCount = 0;
    if (this._translate) instanceCount = this._translate.length / 3;
    else if (this._instanceTransforms) instanceCount = this._instanceTransforms.length / 16;

    if (instanceCount === 0) return;

    prototypeIds.forEach((protoId, protoIndex) => {
      const protoMesh = this._interface.meshes[protoId];
      if (!protoMesh?._geometry || !protoMesh?._mesh?.material) return;
      ...
      if (this._instanceIndices) {
        for (let i = 0; i < this._instanceIndices.length; i++) {
          if (Number(this._instanceIndices[i]) === protoIndex) countForThisProto++;
        }
      } else {
        if (prototypeIds.length === 1) countForThisProto = instanceCount;
        else if (protoIndex === 0) countForThisProto = instanceCount;
      }

      if (!Number.isFinite(countForThisProto) || countForThisProto <= 0) return;
      ...
      for (let i = 0; i < instanceCount; i++) {
        if (this._instanceIndices && Number(this._instanceIndices[i]) !== protoIndex) continue;
        if (!this._instanceIndices && protoIndex !== 0) continue;
        ...
        if (this._instanceTransforms) {
          ...
          mat.fromArray(this._instanceTransforms as ArrayLike<number>, i * 16);
          mat.transpose();
