// @ts-nocheck
import { Bone, Float32BufferAttribute, Matrix4, Quaternion, Skeleton, SkinnedMesh, Vector3 } from 'three';
...
class HydraSkeleton {
  ...
  updateNode(networkId, path, parameters) {
    if (debugPrims) console.log("HydraSkeleton updateNode", networkId, path, parameters);
    
    // Hydra/USD parameters often come with specific names
    // We check for common ones used in UsdSkel
    if (parameters.joints) this._jointNames = parameters.joints;
    if (parameters.bindTransforms) this._bindTransforms = parameters.bindTransforms;
    if (parameters.restTransforms) this._restTransforms = parameters.restTransforms;
  ...
  updateFinished() {
    if (debugPrims) console.log("HydraSkeleton updateFinished", this._id);
    
    if (!this._jointNames || this._jointNames.length === 0) return;
